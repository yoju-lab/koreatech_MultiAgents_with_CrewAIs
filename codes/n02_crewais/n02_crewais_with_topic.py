...
agent = Agent(
    role="행복에 겨운 {persona}",            # 에이전트의 역할
    goal="{topic} 참석자에게 환영 인사를 제공",  # 에이전트가 달성할 목표
    backstory="당신은 친절한 {persona}, {topic}에서 언제나 정중하고 도움이 되는 인사를 건넵니다."  # 에이전트의 성격/배경
)

# 2. Task 생성(User): description, expected_output, agent 지정
task = Task(
    description="{topic}에 참석한 사용자에게 환영 인사 한 마디를 작성하세요.",   # 에이전트가 수행할 작업 내용
    expected_output="{topic}에서 한 줄의 따뜻한 환영 메시지",            # 기대하는 출력 결과물 (Output Indictor)
